...
import { Supplier, STORAGE_KEYS, getData, saveData, updateData, deleteData } from '../utils/storage';
...
export default function Suppliers() {
  ...
  const loadData = () => {
    setSuppliers(getData<Supplier>(STORAGE_KEYS.SUPPLIERS));
  };
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    if (editingId) {
      updateData<Supplier>(STORAGE_KEYS.SUPPLIERS, editingId, { name, phone });
    } else {
      ...
      saveData<Supplier>(STORAGE_KEYS.SUPPLIERS, newSupplier);
    }
  ...
  const handleDelete = (id: string) => {
    if (window.confirm('هل أنت متأكد من حذف هذا المورد؟')) {
      deleteData<Supplier>(STORAGE_KEYS.SUPPLIERS, id);
      loadData();
    }
  };
    ...
    <div className="min-h-screen bg-gray-200 flex justify-center items-center p-0 sm:p-4">
      <div className="w-full h-[100dvh] sm:h-[850px] max-w-[400px] bg-gray-50 sm:rounded-3xl sm:shadow-2xl overflow-hidden flex flex-col relative border-x-0 sm:border-x-[8px] sm:border-y-[16px] border-gray-900">
        ...
        <header className="bg-[#115e59] text-white px-4 py-4 flex items-center justify-between shadow-md z-10">
          ...
          <h1 className="text-lg font-bold flex-1 text-center tracking-wide">الموردين</h1>
        ...
        <main className="flex-1 overflow-y-auto p-4">
          {suppliers.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-gray-400">
              ...
              <p className="text-lg font-semibold">لا يوجد موردين</p>
            </div>
          ) : (
              ...
              {suppliers.map((sup) => (
                <div key={sup.id} className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex justify-between items-center">
                  <div>
                    ...
                    {sup.phone && <p className="text-sm text-gray-500 mt-1" dir="ltr">{sup.phone}</p>}
                  </div>
                  <div className="flex gap-2" dir="ltr">
                    <button onClick={() => handleDelete(sup.id)} className="p-2 text-red-500 hover:bg-red-50 rounded-lg transition-colors">
                      <Trash2 size={20} />
                    </button>
                    <button onClick={() => handleOpenModal(sup)} className="p-2 text-blue-500 hover:bg-blue-50 rounded-lg transition-colors">
                      <Edit size={20} />
                    </button>
                  </div>
                </div>
          ...
          <div className="absolute inset-0 bg-black/50 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
            <div className="bg-white rounded-2xl w-full max-w-sm shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
              <div className="bg-[#115e59] px-4 py-3 flex justify-between items-center text-white">
                <h2 className="font-bold text-lg">{editingId ? 'تعديل بيانات المورد' : 'إضافة مورد جديد'}</h2>
              ...
              <form onSubmit={handleSave} className="p-5 space-y-4">
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-1">اسم المورد <span className="text-red-500">*</span></label>
                  <input 
                    type="text" 
                    required
                    value={name}
